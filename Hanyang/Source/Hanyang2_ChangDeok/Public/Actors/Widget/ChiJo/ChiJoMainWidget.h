...
#include "ChiJoMainWidget.generated.h"
...
class UTextWidget;
class AChiJoWidgetActor;
class USubLevelDescriptButtonWidget;
class UFadeWidget;
class UChiJoSubDescriptWidget;

UCLASS()
class HANYANG2_CHANGDEOK_API UChiJoMainWidget : public UUserWidget
{
	GENERATED_BODY()
public:
	void C_Init();
	...
	void SetWidgetActor(AChiJoWidgetActor* _WidgetActor) { WidgetActor = _WidgetActor; }
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	UChiJoSubDescriptWidget* C_ChiJoSubDescriptWidget;

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	UWidgetAnimation* C_SubDescriptAnim;

private:
	AChiJoWidgetActor* WidgetActor;
