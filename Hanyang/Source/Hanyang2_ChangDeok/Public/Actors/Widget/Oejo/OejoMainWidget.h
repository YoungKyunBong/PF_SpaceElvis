...
#include "OejoMainWidget.generated.h"
...
class UTextWidget;
class AOejoWidgetActor;
class USubLevelDescriptButtonWidget;
class UFadeWidget;
class UOejoSubDescriptWidget;

UCLASS()
class HANYANG2_CHANGDEOK_API UOejoMainWidget : public UUserWidget
{
	GENERATED_BODY()
public:
	void C_Init();
	...
	void SetWidgetActor(AOejoWidgetActor* _WidgetActor) { WidgetActor = _WidgetActor; }
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	UOejoSubDescriptWidget* C_OeJoSubDescriptWidget;

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	UWidgetAnimation* C_SubDescriptAnim;

private:
	AOejoWidgetActor* WidgetActor;
